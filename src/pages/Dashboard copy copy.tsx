...
export default function Dashboard() {
  const { user, deductCredits, updateUser, isServiceEnabled, addWorkHistory, getWorkHistory, creditSettings, addCredits, getServiceCreditCost } = useAuth();
  ...
  useEffect(() => {
    ...
    console.log('Dashboard: Credit settings:', creditSettings);
    
    try {
      if (user) {
        console.log('Dashboard: User found, initializing...');
        setIsLoading(false);
      } else {
  ...
  const services = React.useMemo(() => {
    try {
      return allServices.filter(service => {
        const enabled = isServiceEnabled ? isServiceEnabled(service.id) : true;
        console.log(`Dashboard - Service ${service.id}: enabled = ${enabled}`);
        return enabled;
      });
    } catch (err) {
      console.error('Dashboard: Error filtering services:', err);
      return allServices; // Fallback to all services
    }
  }, [isServiceEnabled]);
  ...
  // Early return if no user (should be handled by ProtectedRoute, but just in case)
  if (!user) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
    ...
    for (const file of newFiles) {
      ...
      if (fileExtension === 'csv') {
        try {
          ...
          const serviceCreditCost = selectedService ? getServiceCreditCost(selectedService) : 1;
          const totalCredits = dataRows * serviceCreditCost;
  ...
  if (!user) {
    console.log('Dashboard: No user found at end of component');
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
  ...
  console.log('Dashboard: Rendering main dashboard for user:', user.name);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{user.services?.length || 0}</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {!showHistory ? (
                ...
                !user || getWorkHistory(user.id).length === 0 ? (
                  ...
                  user && getWorkHistory(user.id).filter(item => item.status === 'completed').map(historyItem => (
        ...
        {user && getWorkHistory(user.id).filter(item => item.status === 'completed').length > 0 && (
          <div className="mt-8 bg-white rounded-xl shadow-lg p-6">
            ...
            {(() => {
              const workHistory = getWorkHistory(user.id).filter(item => item.status === 'completed');
              return (
